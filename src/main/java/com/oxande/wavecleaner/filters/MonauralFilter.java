...
public class MonauralFilter extends UGen {
	...
	// * the beginning.
	// *
	// * @param millis int: the position to place the "playhead", in
	// milliseconds
	// *
	// * @related FilePlayer
	// */
	...
	// // change the position in the stream invalidates our buffer, so we read a
	// new buffer
	// fillBuffer();
	// }
	//
	// /**
	// * Skips <code>millis</code> from the current position.
	// <code>millis</code>
	// * can be negative, which will make this skip backwards. If the skip
	// amount
	...
	// pos = length();
	// }
	// //Minim.debug("AudioPlayer.skip: skipping " + millis + " milliseconds,
	// new position is " + pos);
	// cue( pos );
	// }
	//
	...
	/**
	 * Calling close will close the AudioRecordingStream that this wraps, which
	 * is proper cleanup for using the stream.
	 * 
	 * @related FilePlayer
	 */
	public void close() {
